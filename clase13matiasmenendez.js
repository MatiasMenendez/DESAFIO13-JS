...
const inoxidable = new Material('inoxidable', 1300, 1400, true, 7.2, medida1I, medida2I, espesorI);
const hierro = new Material('hierro', 330, 430, true, 7.2, medida1H, medida2H, espesorH);
const bronce = new Material('bronce', 500, 600, false, 7,2, medida1B, medida2B, espesorB);
...
function calcularPresupuestoPesosPH(){
    let precioHierroP = ((medida1PH.value + medida1PH.value)/1000) * (medida2PH.value/1000) * (medida3PH.value/1000) * espesorPH.value * pesoEspecifico * hierro.precioPorKiloP;
    document.querySelector("#precioPesosPH").value = precioHierroP + "pesos"}
    
    function calcularPresupuestoDolarPH(){
    let precioHierroPD = ((medida1PH.value + medida1PH.value)/1000) * (medida2PH.value/1000) * (medida3PH.value/1000) * espesorPH.value * pesoEspecifico * hierro.precioPorKiloP / valorDolar;
    document.querySelector("#precioDolaresPH").value = precioHierroPD + "dolares"}
    
    
    function calcularPresupuestoPesosPI(){
    let precioInoxP = ((medida1PI.value + medida2PI.value)/1000) * (medida3PI.value/1000) * espesorPI.value * pesoEspecifico * inoxidable.precioPorKiloP;
    document.querySelector("#precioPesosPI").value = precioInoxP + "pesos"}
      
    function calcularPresupuestoDolarPI(){
    let precioInoxPD = ((medida1PI.value + medida2PI.value)/1000) * (medida3PI.value/1000) * espesorPI.value * pesoEspecifico * inoxidable.precioPorKiloP / valorDolar;
    document.querySelector("#precioDolaresPI").value = precioInoxPD + "dolares"}
    
    
    function calcularPresupuestoPesosPB(){
    let precioBronceP = ((medida1PB.value + medida2PB.value)/1000) * (medida3PB.value/1000) * espesorPB.value * pesoEspecifico * bronce.precioPorKiloP;
    document.querySelector("#precioPesosPB").value = precioBronceP + "pesos"}
      
    function calcularPresupuestoDolarPB(){
    let precioBroncePD = ((medida1PB.value + medida2PB.value)/1000) * (medida3PB.value/1000) * espesorPB.value * pesoEspecifico * bronce.precioPorKiloP / valorDolar;
...
/*Primeros dos botones pasados a JQuery */

const botonPresupuestarH = document.querySelector("#botonPresupuestarH")

$("#botonPresupuestarH").on("click", (evt) => {
 evt.preventDefault();
 $(".prueba1").fadeIn(2000)
